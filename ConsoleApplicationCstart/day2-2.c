...
/*
	...
	*/
int main(void) {
	// 조립 과정 : 10진 정수, 각 자리 값을 합친다
	// 2진수 연산으로 각 자리 값의 사용 여부를 표시한다
	int num = 0;
	int flag = 0; // 상태와 조건 구분을 위한 변수 
	for (int i = 0; i < 10; ++i) {
		flag = 0;
		num = rand()%9+1;
...
int main_02(void) {
	// 부호 사용
	unsigned int n = 0xA5;//2진수 값을 쓸수 없다
	printBits(n);// 정수를 2진수로 출력하는 함수
	// n 변수의 [2]번 비트를 0으로 만드세요
	unsigned int a = (1 << 2);
	unsigned int c = ~(1 << 2);//[2]번을 제외한 나머지는 1
	n = n & c;// c변수의 1인 비트 자리는 모두 n의 값이 그대로
	printBits(n);
